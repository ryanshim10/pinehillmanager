...
class PaymentsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val units = unitDao.getAllUnits().stateIn(viewModelScope, SharingStarted.WhileSubscribed(5_000), emptyList())
    val rules = ruleDao.getRules().stateIn(viewModelScope, SharingStarted.WhileSubscribed(5_000), emptyList())

    private val _selectedMonth = MutableStateFlow(currentMonth())
    val selectedMonth: StateFlow<String> = _selectedMonth.asStateFlow()
    val monthPayments = _selectedMonth.flatMapLatest { paymentDao.getPaymentsByMonth(it) }
    ...
    val historyPayments = _historyUnitId.flatMapLatest { unitId ->
        if (unitId.isNullOrBlank()) flowOf(emptyList()) else paymentDao.getMatchedPaymentsByUnitSince(unitId, twoYearsAgo())
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5_000), emptyList())
    ...
    fun selectHistoryUnit(unitId: String?) { _historyUnitId.value = unitId }
    ...
    fun matchPaymentToUnit(payment: Payment, unitId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (sender.isBlank()) {
                paymentDao.updatePayment(payment.copy(unitId = unitId, status = PaymentStatus.PAID, statusOverride = true))
            ...
            val contract = database.contractDao().getContractsByUnitNow(unitId)
                .firstOrNull { it.monthlyRent == 0L || it.monthlyRent == payment.amount }
    ...
    private suspend fun applyRule(rule: PaymentMatchRule): Int {
        ...
        paymentDao.getPendingPaymentsNow().forEach { payment ->
            ...
            val matches = parsed.bankName == rule.bankName &&
                ...
                payment.amount == rule.amount && SmsParser.dayOfMonth(payment.paidAt) == rule.dayOfMonth
            if (matches) {
                paymentDao.updatePayment(payment.copy(unitId = rule.unitId, status = PaymentStatus.PAID, statusOverride = true))
                count++
            }
        }
        return count
    ...
    companion object {
        private fun currentMonth(): String = SimpleDateFormat("yyyy-MM", Locale.KOREA).format(Date())
        private fun oneYearAgo(): Long = Calendar.getInstance().apply { add(Calendar.YEAR, -1) }.timeInMillis
        private fun twoYearsAgo(): Long = Calendar.getInstance().apply { add(Calendar.YEAR, -2) }.timeInMillis
        fun formatWon(amount: Long): String = String.format(Locale.KOREA, "%,d원", amount)
        fun bankName(payment: Payment): String = payment.rawSms?.let { SmsParser.parseDepositSms(it)?.bankName }.orEmpty()
