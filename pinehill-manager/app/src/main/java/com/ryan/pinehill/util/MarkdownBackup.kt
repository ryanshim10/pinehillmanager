...
object MarkdownBackup {
    ...
    suspend fun restore(db: AppDatabase, markdown: String): RestoreResult {
        ...
        if (root.optString("format") != "pinehill-manager") {
            return RestoreResult(false, "지원하지 않는 백업 형식입니다.")
        }
    ...
    private fun unitToJson(u: RentalUnit) = JSONObject().apply {
        put("unitId", u.unitId)
        put("roomNo", u.roomNo)
        put("floor", u.floor)
        put("status", u.status.name)
        put("roomType", u.roomType ?: JSONObject.NULL)
        put("targetPrice", u.targetPrice ?: JSONObject.NULL)
        put("createdAt", u.createdAt)
        put("updatedAt", u.updatedAt)
    }

    private fun unitFromJson(j: JSONObject) = RentalUnit(
        unitId = j.getString("unitId"),
        roomNo = j.getInt("roomNo"),
        floor = j.getInt("floor"),
        status = UnitStatus.valueOf(j.optString("status", UnitStatus.RENTED.name)),
        roomType = j.optNullableString("roomType"),
        targetPrice = j.optNullableString("targetPrice"),
    ...
    private fun tenantFromJson(j: JSONObject) = Tenant(
        tenantKey = j.getString("tenantKey"),
    ...
    private fun paymentToJson(p: Payment) = JSONObject().apply {
        ...
        put("tenantKey", p.tenantKey ?: JSONObject.NULL)
        put("unitId", p.unitId)
        put("month", p.month)
        put("paidAt", p.paidAt ?: JSONObject.NULL)
        put("amount", p.amount)
        put("senderName", p.senderName ?: JSONObject.NULL)
        put("source", p.source.name)
        put("status", p.status.name)
        put("statusOverride", p.statusOverride)
        put("rawSms", p.rawSms ?: JSONObject.NULL)
        put("createdAt", p.createdAt)
    }

    private fun paymentFromJson(j: JSONObject) = Payment(
        paymentId = j.optLong("paymentId"),
        tenantKey = j.optNullableString("tenantKey"),
        unitId = j.optString("unitId"),
        month = j.optString("month"),
        paidAt = j.optNullableLong("paidAt"),
        amount = j.optLong("amount"),
        senderName = j.optNullableString("senderName"),
        source = PaymentSource.valueOf(j.optString("source", PaymentSource.MANUAL.name)),
        status = PaymentStatus.valueOf(j.optString("status", PaymentStatus.PENDING.name)),
        statusOverride = j.optBoolean("statusOverride"),
        rawSms = j.optNullableString("rawSms"),
        createdAt = j.optLong("createdAt", System.currentTimeMillis())
    )

    private fun expenseToJson(e: Expense) = JSONObject().apply {
        put("expenseId", e.expenseId)
        put("spentAt", e.spentAt)
        put("amount", e.amount)
        ...
        put("rawSms", e.rawSms ?: JSONObject.NULL)
        put("createdAt", e.createdAt)
    }

    private fun expenseFromJson(j: JSONObject) = Expense(
        expenseId = j.optLong("expenseId"),
        spentAt = j.optLong("spentAt"),
        amount = j.optLong("amount"),
        category = ExpenseCategory.valueOf(j.optString("category", ExpenseCategory.OTHER.name)),
        memo = j.optString("memo"),
        unitId = j.optNullableString("unitId"),
        month = j.optString("month"),
        source = ExpenseSource.valueOf(j.optString("source", ExpenseSource.MANUAL.name)),
        rawSms = j.optNullableString("rawSms"),
        createdAt = j.optLong("createdAt", System.currentTimeMillis())
    )

    private fun contractToJson(c: ContractRecord) = JSONObject().apply {
        put("contractId", c.contractId)
        put("tenantName", c.tenantName)
        put("tenantPhone", c.tenantPhone)
        put("unitId", c.unitId)
        put("deposit", c.deposit)
        put("monthlyRent", c.monthlyRent)
        put("paymentDay", c.paymentDay)
        put("startDate", c.startDate)
        put("endDate", c.endDate)
        put("documentUri", c.documentUri)
        put("sourceName", c.sourceName)
        put("rawOcrText", c.rawOcrText)
        put("notes", c.notes)
    ...
    private fun contractFromJson(j: JSONObject) = ContractRecord(
        contractId = j.optLong("contractId"),
        tenantName = j.optString("tenantName"),
        tenantPhone = j.optString("tenantPhone"),
        unitId = j.optString("unitId"),
        deposit = j.optLong("deposit"),
        monthlyRent = j.optLong("monthlyRent"),
        paymentDay = j.optInt("paymentDay"),
        startDate = j.optString("startDate"),
        endDate = j.optString("endDate"),
        documentUri = j.optString("documentUri"),
        sourceName = j.optString("sourceName"),
        rawOcrText = j.optString("rawOcrText"),
    ...
    private fun ruleFromJson(j: JSONObject) = PaymentMatchRule(
        ruleId = j.optLong("ruleId"),
        bankName = j.optString("bankName"),
        senderName = j.optString("senderName"),
        amount = j.optLong("amount"),
        dayOfMonth = j.optInt("dayOfMonth"),
        unitId = j.optString("unitId"),
        tenantName = j.optString("tenantName"),
